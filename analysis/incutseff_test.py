...
if run_all:
	...
	event_dic = {}
	with open('filelist/andrev2plus.txt') as f:
		rootfilesandre = f.readlines()
	with open('filelist/tres40.txt') as f:
	...
	#rootfilesx = rootfilesx + rootfilesx+ rootfilesx + rootfilesx + rootfilesx
		...
		#print rootfiles
		rootfiles1 = [rootprefix+"1mm/%s"%(x.strip()) for x in rootfiles if ("_ctau-1p0_" in x or "_ctau-1_" in x) ]
		rootfiles2 = [rootprefix+"10mm/%s"%(x.strip()) for x in rootfiles if ("_ctau-10p0_" in x or "_ctau-10_" in x) ]
...
def makehist(events,life):
	...
	for iev,event in enumerate(events):
		if (iev %100 ==0):
			print "Lifetime: %s, Event %d: run %6d, lumi %4d, event %12d" % (life,iev,event.eventAuxiliary().run(), event.eventAuxiliary().luminosityBlock(),event.eventAuxiliary().event())
...
if not run_all:
	...
	if run_lifetimes:
        ...
        lifelist = ["ctau 100mm"]#,"ctau 10mm","ctau 100mm","ctau 1000mm"]
        plots_dicmass = {}
        plots2d_dicmass = {}
        plotseff_dicmass = {}
        plots_diclife = {}
	...
	counts_dicmass ={}
        ...
        for mass_split,key in zip(mass_splitlist1,mass_splitlist2):
                ...
                for num,life in enumerate(lifelist):
                        all_ret = makehist(event_dic[mass_split][num],life)
...
def saveplots(plots_dic,plotseff_dic,plots2d_dic,counts_dic,plotsmuons_dic,olist):
	for key in plots_dic:
		ofile = "output_test/%s_cutsel.pdf"%(key.replace(" ","_"))
...
#			))
##counts_dic[key][i][0][1],counts_dic[key][i][1][1],counts_dic[key][i][0][2],counts_dic[key][i][1][2],counts_dic[key][i][0][3],counts_dic[key][i][1][3],counts_dic[key][i][0][4],counts_dic[key][i][1][4]))
...
#			nevents_textrec[i].Draw() 
		...
		pp.Print(ofile+"(")#utput/%s_cutsel.pdf("%(key.replace(" ","_")))
		...
		for i,h in enumerate(plots2d_dic[key]):
			for hi in h:
				...
				pp.Print(ofile)
				#pp.Print("output/%s_cutsel.pdf"%(key.replace(" ","_")))
		for mu,submu in plotsmuons_dic[key]:
			#print mumass
			#for mu, submu in zip(mumass[0],mumass[1]):
			#	print mu, submu
				leg = {}
				for sel in range(5):
					leg[sel] = ROOT.TLegend(.70,.70,.85,.85)
					drawtwo(mu[sel],submu[sel],key,olist,pp,sel,leg[sel])
				pp.Print(ofile)
				del leg		
		pp.Clear()	
		pp.cd(0)	
		...
		for h1,h2 in zip(plotseff_dic[key][0],plotseff_dic[key][1]):
			leg = ROOT.TLegend(.70,.70,.85,.85)
			#drawall(h1,h2,h3,h4,None,key,olist,pp,-1,leg)
			drawtwo(h1,h2,key,olist,pp,-1,leg)
			pp.Print(ofile)
			#pp.Print("output/%s_cutsel.pdf"%(key.replace(" ","_")))
			del leg
		pp.Print(ofile+")")
...
saveplots(plots_diclife,plotseff_diclife,plots2d_diclife,counts_diclife,plotsmuons_diclife,mass_splitlist2)
